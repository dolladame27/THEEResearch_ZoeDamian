...
simulate_one_gen1 <- function(N_a, N_A, w_a, w_A,  R_rate, sigma_env_a, sigma_env_A, K, A) {
  ...
  allee <- (Ntot / (Ntot + A))^0.5  #Allee effect affecting total pop. 
  
  env_a <- max(0, rnorm(1, mean=1, sd= sigma_env_a))  #env.eff. already normal distributed
...
####--------------------------------------- 
#### plotting extinction without rescue: varying sigma env
####---------------------------------------
...
for (choose_sig in var_sigma) {
  ...
  results <- replicate(nrep, run_sim_c(100,0,1.05,1.2,0,t_max, sigma_env_a=choose_sig, sigma_env_A=0.1, 5000, 20))
  # print(results)
...
####------------------------------------------------
#### plot extinction without rescue: varying fitness
####------------------------------------------------
...
for (choose_w_a in var_w_a) {
  ...
  results <- replicate(nrep, run_sim_c(100,0,w_a=choose_w_a,1.2,0,tmax, sigma_env_a= 0.3, sigma_env_A=0, K, A))
  # print(results)
...
## draw env values
env_a <- max(0, rnorm(1, mean=1, sd= sig_a))  #env.eff. already normal distributed
...
max_gen <- 100
# create your simulation data
output<- simulate_diploid(100, 20, 0, 1, 1.5, 1.25, 0, max_gen, 0.1, 0.3, sig_Aa=0.2, K=2000, A=20)
# show the last few lines of the data table
print(tail(output))
...
plot(x_range,
     ...
     xlab = "Generation",
     ylab = "Population size", 
     title= "Lynx pardinus diploid population rescue")
...
legend("topright",
       ...
       title = "diploid Lynx pardinus population rescue",
...
### Now what happens if the Mutation is only on one Allele? (Heterozygous)
...
### what fraction of mutants rescues the initial population (at 80%?)
...
## used to estimate extinction frequency in a given number of trials when stochasticity in ecological parameters 
## (in this case environmental fluctuation) are involved
...
if (nrow(cand) > 0) {
  ...
  message(sprintf("Minimal number of mutants for ≥ %.0f%% rescue: %d individuals",
